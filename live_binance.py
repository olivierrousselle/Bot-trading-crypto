...
fiatSymbol = 'BUSD'
coin = 'ETH'
pairSymbol = pairSymbol = coin + fiatSymbol
trixLength = 9
trixSignal = 21
...
cryptoAmount = float(client.get_asset_balance(asset=coin)['free'])
...
if buyCondition(df.iloc[-2], df.iloc[-3]):
    if float(fiatAmount) > 5:
        ...
        buyOrder = client.order_market_buy(symbol=pairSymbol, quantity=quantityBuy)
        print("BUY", buyOrder)
    else:
        pass
        print("If you  give me more USD I will buy more",coin) 

elif sellCondition(df.iloc[-2], df.iloc[-3]):
    if float(cryptoAmount) > minToken:
        sellOrder = client.order_market_sell(symbol=pairSymbol, quantity=convert_amount_to_precision(pairSymbol, cryptoAmount))
        print("SELL", sellOrder)
    else:
        pass
        print("If you give me more",coin,"I will sell it")
else :
    pass
